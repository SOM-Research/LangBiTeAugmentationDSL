...
import { DocumentationGenerator } from './langbite-augmentation-dsl-documentation-generator.js';
import fs from 'fs';
...
export function activate(context: vscode.ExtensionContext): void {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('langbite-augmentation-dsl.generateJson', async () => {
        await generateJsonService(context);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('langbite-augmentation-dsl.saveJsonDocument', async () => {
...
function startLanguageClient(context: vscode.ExtensionContext): LanguageClient {
    ...
    const fileSystemWatcher = vscode.workspace.createFileSystemWatcher('**/*.{augml,aug}');
    context.subscriptions.push(fileSystemWatcher);

    // Options to control the language client
    const clientOptions: LanguageClientOptions = {
        documentSelector: [{ scheme: 'file', language: 'ethics-ml' }],
        synchronize: {
...
async function generateJsonService(context: vscode.ExtensionContext) {
    let title : string = 'JSON Test Scenario';
